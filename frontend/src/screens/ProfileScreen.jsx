...
const ProfileScreen = ()=>{
    ...
    const { data:myOrders, isLoading:isMyordersLoading, error:errorInGettingMyOrders } = useGetMyOrdersQuery();
    ...
    return (
        <Row>
            ...
            <Form onSubmit={handleSubmit}>
                ...
                <Button variant="primary" type="submit" disabled={isUpdateProfileLoading}>Update</Button>
            </Form>
            </Col>
            <Col md={9}>
                <h2>My Orders</h2>
                {isMyordersLoading ? (
                    ...
                    errorInGettingMyOrders ? (
                        <Message variant="danger">Unable to get your orders. Try Again.</Message>
                    ) : (
                        <Table striped hover borderless responsive className="table-sm">
                            ...
                            <tbody>
                                {myOrders?.map((order)=>{
                                    const { _id, createdAt, isPaid, paidAt, isDelivered, totalPrice } = order;
                                    return (
                                        <tr key={_id}>
                                            <td>{_id}</td>
                                            <td>{createdAt}</td>
                                            ...
                                            <td>
                                                <Button variant="light" onClick={()=> redirectPage(_id)}>Details</Button>
                                            </td>
                                        </tr>
                                    )
                                })}
